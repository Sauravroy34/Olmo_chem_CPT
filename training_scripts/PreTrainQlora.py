"""
PreTrainQlora.py — QLoRA (4-bit quantized LoRA) continual pre-training for OLMo-7B on chemistry SMILES.

Usage examples with torchrun
============================

    torchrun --nproc_per_node=<NUM_GPUS> PreTrainQlora.py \
        --hf_token "hf_XXXXX" \
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="QLoRA (4-bit quantized LoRA) continual pre-training for OLMo-7B on chemistry SMILES.",
    ...
    parser.add_argument("--output_dir", type=str,
                        default="./olmo_chem_lora_cpt_QLoRA_r64_alpha128",
                        help="Local directory for checkpoints and adapter (default: ./olmo_chem_lora_cpt_QLoRA_r64_alpha128).")
...
def setup_model(base_model: str, lora_r: int, lora_alpha: int, lora: bool = True):
    print_main(f"Loading base model with QLoRA (4-bit): {base_model}")
    ...
    if lora:
        ...
        print_main("Applying QLoRA adapter configuration...")
...
def main():
    ...
    if is_main_process:
        ...
        card_data = ModelCardData(
            ...
            license="apache-2.0",
            library_name="peft",
            tags=["chemistry", "smiles", "olmo", "causal-lm", "qlora", "peft", "4bit"],
...
base_model = AutoModelForCausalLM.from_pretrained(
    "{args.base_model}", quantization_config=bnb_config, trust_remote_code=True
)
...
- QLoRA adapters only; requires the base model [{args.base_model}](https://huggingface.co/{args.base_model}) loaded in 4-bit to use.
