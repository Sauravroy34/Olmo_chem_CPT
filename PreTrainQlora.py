...
# QLoRA config constants
LORA_R = 64
LORA_ALPHA = 128

DATASET_NAME = "Codemaster67/Causal_lm_chemistry_1M_rows"        
OUTPUT_DIR = "./olmo_chem_lora_cpt_QLoRA_r64_alpha128"
...
def setup_model(model_id=BASE_MODEL, lora=True):
    print_main(f"Loading base model with QLoRA (4-bit): {model_id}")
...
def main():
    ...
    wandb_project = f"OLMo_QLoRA_r{LORA_R}_a{LORA_ALPHA}_{run_timestamp}"
    wandb_run_name = f"qlora_r{LORA_R}_a{LORA_ALPHA}_lr{LEARNING_RATE}_{run_timestamp}"
...
- QLoRA adapters only; requires the base model [{BASE_MODEL}](https://huggingface.co/{BASE_MODEL}) loaded in 4-bit to use.
